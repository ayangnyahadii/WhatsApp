...
handler.before = async function (m) {
    ...
    if (m.isGroup && global.db.data.chats[m.chat].expired != 0) {
        if (new Date() * 1 >= global.db.data.chats[m.chat].expired) {
            this.reply(m.chat, `Waktu Sewa Telah Habis, Saatnya Saya Keluar Dari Group ini? Ingin Perpanjangan bot? Anda Bisa Mengghubungi Nomor Di Bawah Ini`, null).then(() => {
